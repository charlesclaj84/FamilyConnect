 ...
 * array for a repeated key and `/community/announcements?q=a&q=b` is a URL anybody can send. `q` typed
 * as `string` was a crash on `/community/updates` — `sanitizeUpdatesQuery` calls `.trim()` — while
 ...
 * 2026-08-19, and still one: `/community/updates` redirects here, which is why the key did not become
 ...
 * the same shape: `getUpdatesArchive()` runs `requireRead('community/updates')` itself and narrows the
 ...
 * `/community/announcements` would resolve against the `announcements` key alone: a caller holding ONLY
...
export default async function AnnouncementsPage({ searchParams }: Props) {
  ...
  await requireFamilyActive(user.id, 'community/announcements')
  await requireTier(user.id, 'community/announcements')
  ...
  const [mayViewBoard, mayViewUpdates, mayViewBirthdays] = await Promise.all([
    ...
    can(user.id, 'community/updates', 'view'),
    ...
    // over a refusal.
    can(user.id, 'community/announcements/birthdays', 'view'),
  ])
  ...
  const [board, archive, birthdays] = await Promise.all([
    mayViewBoard
      ? Promise.all([
        ...
        canAny(user.id, 'community/announcements', 'edit'),
        ...
        scopeFor(user.id, 'community/announcements', 'delete'),
