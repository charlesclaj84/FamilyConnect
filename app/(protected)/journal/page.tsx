...
/**
 * The Journal: an office's notes, read and written by whoever holds it.
 *
 ...
 *
 * The substance of the ask is delivered where it belongs: the DATA. Four policies on
 * `position_journal_entries` test `auth_holds_family_role`, so a member with no office reads
 * nothing and writes nothing whatever their template says — and this page renders a sentence
 * explaining that rather than an error.
 *
 * ── `requireView('journal')` IS THE PAGE'S GATE AND GATES ONLY THE SCREEN ──────────
 * `journal:view` defaults to 'everyone', so what it is FOR is letting a family switch the
 * Journal off entirely. It decides nothing about which entries anybody reads — no policy on
...
export default async function JournalPage() {
  ...
  return (
    <PageShell className="space-y-8">
      <div>
        <h1 className="mb-1 text-3xl font-bold">Journal</h1>
      ...
      {offices.length === 0 ? (
        ...
        <div className="rounded-xl border bg-card px-4 py-12 text-center">
          ...
          <p className="text-sm text-muted-foreground">
            The Journal is for members who hold an office, and you do not hold one yet.
          </p>
