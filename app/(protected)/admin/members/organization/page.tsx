 ...
 *     so the `/admin/members/organization` entry in `lib/features.ts` is what keeps the key
 ...
 *     The chapter that documents this screen carries `route: '/admin/members/organization'`, so
 ...
 * `auth_permission('admin/members/organization', …)`. Re-keying a pane is a nice tidy-up in TypeScript
 ...
 * page does not, and the reason is the one `/admin/members/approvals` already records: it reads
 * nothing, renders nothing and holds no data — it rewrites a URL. `/admin/members` gates
 ...
 *
 * The tab parameter is `?tab=organization` because that is the convention `/admin/members`
