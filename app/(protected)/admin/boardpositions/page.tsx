 ...
 * the screens did. `/admin/users` resolves each one on its own and renders only what the
 * caller holds, so the pane can arrive with one half, the other, or both.
 *
 * ── WHY THE ROUTE STAYS ─────────────────────────────────────────────────────────────
 * The same three reasons `/admin/chapters` records, and all three apply here unchanged:
 *
 ...
 *     the `/admin/boardpositions` entry in `lib/features.ts` is what keeps the key
 ...
 *     the chapter documenting this screen carries `route: '/admin/boardpositions'`.
 *
 ...
 * `auth_permission('admin/boardpositions', …)`. Re-keying a pane is a tidy-up in TypeScript
 ...
 * The reason `/admin/chapters` and `/admin/approvals` already record: this reads nothing,
 * renders nothing and holds no data — it rewrites a URL. `/admin/users` gates itself on all
 ...
 * `/admin/users` resolves the pane on `canAny`, so an 'own' holder gets no pane rather than an
