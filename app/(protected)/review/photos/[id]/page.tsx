...
export default async function PhotoCollectionPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  await requireView(user.id, 'review/photos')
  ...
  return (
    <PageShell className="space-y-6">
      <div>
        <Link href="/review/photos" className="text-sm text-muted-foreground hover:text-foreground flex items-center gap-1 mb-4">
      ...
      <PhotoCollectionGallery
        ...
        isAdmin={await can(user.id, 'review/photos', 'delete')}
