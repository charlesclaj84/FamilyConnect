...
interface Props {
  ...
  money: Money
  /**
   * The reader's translator. A PROP for `money`'s reason above, and this card SHIPPED with
 ...
 */
export function AccountPnLCard({ data, money, t }: Props) {
    ...
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div className="rounded-2xl border bg-card p-5 space-y-2">
          ...
          <p className="text-3xl font-bold">{money(data.totalCollectedCents)}</p>
          <div className="text-xs text-muted-foreground space-y-0.5">
            ...
            <p className="flex items-center justify-between gap-2"><span>{t('pnl.duesAndDonations')}</span><span className="font-medium text-foreground">{money(data.totalIncomeCents)}</span></p>
            <p className="flex items-center justify-between gap-2"><span>{t('pnl.direct')}</span><span className="font-medium text-foreground">{money(data.totalContributionsCents)}</span></p>
          </div>
        </div>
        ...
        <div className="rounded-2xl border bg-card p-5 space-y-2">
          ...
          <p className="text-3xl font-bold">{money(data.totalExpenseCents)}</p>
            ...
            <div className="text-xs text-muted-foreground space-y-0.5">
              <p className="flex items-center justify-between gap-2">
                ...
                <span className="font-medium text-foreground">
                  {money(data.totalExpenseCents - data.totalFeesCents)}
                </span>
              </p>
              <p className="flex items-center justify-between gap-2">
                <span>{t('pnl.processingFees')}</span>
                <span className="font-medium text-foreground">
                  {money(data.totalFeesCents)}
                </span>
              </p>
            </div>
          ) : (
              ...
              {data.routing.map(r => (
                <details key={r.fundId} className="px-4 py-2.5">
                  <summary className="flex items-center justify-between cursor-pointer">
                    ...
                    <span className="text-sm font-semibold text-brand-affirm">{money(r.contributedCents)}</span>
                  </summary>
                  <ul className="mt-2 space-y-0.5 ps-1">
                    {r.bySource.map((s, i) => (
                      <li key={i} className="flex justify-between text-xs text-muted-foreground px-1 py-0.5">
                        <span>{s.label}</span>
                        <span className="ms-4 shrink-0">{money(s.cents)}</span>
                      </li>
                    ))}
                  </ul>
                </details>
                ...
                <div key={f.fundId} className="flex items-center justify-between gap-3 px-4 py-2.5">
                  <div className="min-w-0">
                    ...
                    <p className="text-xs text-muted-foreground">
                      ...
                      In {money(f.contributedCents)} · Disbursed {money(f.disbursedCents)}
                    </p>
                  </div>
