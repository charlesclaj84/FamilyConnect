 ...
 *
 * ── AN EDGE IS AN EDGE, AND THE WALK ASKS NOTHING ELSE OF IT ───────────────────────
 * Every parent or child link belongs on the canvas whatever the family calls it. Which
 * relatives are on screen at all is the Bloodline filter's job, and the caller applies it
 * by handing this function an already-filtered PEOPLE list — one rule, applied once,
 * rather than two places deciding who is visible.
 *
 * There used to be a `kind` on `TreeLink` and a paragraph here saying this walk ignored
 * it. `20260902000000` removed the column: the bloodline is `people.is_bloodline` now, so
 * there is nothing on an edge left to ignore.
 */
